module AjaxfulRating # :nodoc:
  ...
  module Locale
    ...
    def i18n(key, value = nil, dimension = nil)
      key = if key == :current
      ...
      key = :no_ratings if key == :hover && ((value || show_value) == 0)
      star =  (value ? ".star#{value.to_i}" : "")
      dim = dimension ? ".#{dimension.to_s}" : ""
      default = DEFAULTS[key]
      key = "ajaxful_rating.helper#{dim}.#{key}#{star}"
      I18n.t(key, :value => (value || show_value),
        :max => rateable.class.max_stars(options[:dimension]), :default => default)
    end
  end
end
